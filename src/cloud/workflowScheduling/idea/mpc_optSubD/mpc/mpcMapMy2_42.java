...
//最终版
//与MySubD2.41区别，41出口任务的subD不能到D，增加了depth=2时，llevel=0
//MySubD2.42 puRank，和MySubD2.3的区别仅仅是在rank上。入口任务预测entrySize的double；预测步数不小于0.1，不大于0.2
public class mpcMapMy2_42 implements Scheduler {
	...
	public mpcMapMy2_42(double theta){
		this.theta = theta;
	}
	public mpcMapMy2_42(){
...
//		VM.resetInternalId(); //算法开始没有Solution solution = new Solution()，则调度算法前要调用
...
//		predictTaskNum = enrySize; //mpcMap的入口任务预测entrySize布
		...
		for(int i = 1; i < tasks.size(); i++){	
			...
			if(task.getDepth() == (this.workflowDepth-1)) {
//				if(i <= enrySize) {
					predictTaskNum = enrySize*2;
