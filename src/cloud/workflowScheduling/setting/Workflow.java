...
//		System.out.println("An approximate value for maximum parallel number: " + maxParallel);  
	...
	public void calcTaskLevels(){ //L-ACO文章中的bLvel计算以及staticLevel(没有考虑任务间的传输时间)计算
...
//			System.out.println(t.getName() +"\t"+t.getbLevel());
	...
	public void calcTaskLevelsAverage(){ //L-ACO文章中的bLvel计算以及staticLevel(没有考虑任务间的传输时间)计算
		double speed = VM.SPEEDS[VM.FASTEST];
		...
		for(int j= this.size()-1; j>=0; j--){ //从出口任务开始计算upward rank
			...
			task.setbLevel(bLevel + task.averageET); //用平均执行时间计算
//			task.setbLevel(bLevel + task.getTaskSize() / speed); //用最快执行时间计算
			task.setsLevel(sLevel + task.getTaskSize() / speed);
		}
...
//			System.out.println(t.getName() +"\t"+t.getbLevel());
	...
	/**
	 * 根据upward rank和level上下时间比例*任务时间计算rank 2.1
	 * @param taskLevelSet
	 * @param ETSubDlevel
	 */
/*	public void calcTaskBLLevels21(Map<Integer, List<Task>> taskLevelSet, List<Double> ETSubDlevel) { 
		double speed = VM.SPEEDS[VM.FASTEST];
	    ...
	    while(tls.hasPrevious()) {  //逆序遍历	//从出口任务开始计算
...
//				if(depth == 1 || depth == taskLevelSet.size())
					lLevel = 0;
				else if(depth == taskLevelSet.size())
					lLevel = ETSubDlevel.get(depth-1) * task.getTaskSize() / speed;
				else
					lLevel = ETSubDlevel.get(depth-1) / ETSubDlevel.get(depth) * task.getTaskSize() / speed;
//					lLevel = ETSubDlevel.get(depth-1) / ETSubDlevel.get(depth);
				
				task.setbLevel(bLevel + lLevel + task.getTaskSize() / speed);
	        }
	    }	
	}*/
	...
	/**
	 * 根据upward rank和level上下个数比例计算rank 2.0
	 * @param taskLevelSet
	 */
	public void calcTaskBLLevels20(Map<Integer, List<Task>> taskLevelSet) { 
	    ...
	    while(tls.hasPrevious()) {  //逆序遍历	//从出口任务开始计算
	        ...
	        for(Task task : tasks) {
				...
				//计算任务的level比例
				double lLevel = 0;
//				if(depth == 1 || depth == taskLevelSet.size()) //2.0.1该效果不如只有一个条件
				if(depth == 1) //2.0.0
					lLevel = 0;
				else
					lLevel =  taskLevelSet.get(Integer.valueOf(depth-1)).size()/ tasks.size();
				
				task.setbLevel(bLevel + lLevel + task.getTaskSize() / speed);
//				System.out.println(bLevel + lLevel + task.getTaskSize() / speed);
	        }
	    }	
	}
	
	/**
	 * 根据upward rank和level上下个数差 比例计算rank 2.0.2
	 * @param taskLevelSet
	 */
	public void calcTaskBLLevels20(Map<Integer, List<Task>> taskLevelSet, List<Double> ETSubDlevel, double total) {
	    ...
	    while(tls.hasPrevious()) {  //逆序遍历	//从出口任务开始计算
	        ...
	        for(Task task : tasks) {
				...
				//计算任务的level比例
				double lLevel = 0;
//				if(depth == 1 || depth == taskLevelSet.size()) //该效果不如只有一个条件
				if(depth == 1) 
					lLevel = 0;
				else
//					lLevel =  Math.max(ETSubDlevel.get(depth)/ total, 0);
					lLevel =  Math.abs(ETSubDlevel.get(depth))/ total;
//				if(lLevel != 0)
//					lLevel = 1;
...
//						double d = 1-Math.pow(1.5, -et / tt);	//网络传输时间越大，d取值越接近于0
//						if(d<random())
//							flag = 0;
//					}
//					
...
//					lLevel =  taskLevelSet.get(Integer.valueOf(depth-1)).size()/ tasks.size();
					lLevel =  tasks.size()/taskLevelSet.get(Integer.valueOf(depth-1)).size();
				if(lLevel >= 1)
					lLevel = 0;
				else
					lLevel = 1;
				...
				task.setbLevel(bLevel + lLevel + task.getTaskSize() / speed);
//				task.setpURank(pURank + task.getTaskSize() / speed + lLevel);
//				System.out.println(bLevel + lLevel + task.getTaskSize() / speed);
	        }
	    }	
	}
	...
	//0或1
		public void calcTaskPURank22(Map<Integer, List<Task>> taskLevelSet) { 
			double speed = VM.SPEEDS[VM.FASTEST];
...
//					lLevel =  taskLevelSet.get(Integer.valueOf(depth-1)).size()/ tasks.size();
					lLevel =  Math.pow(theta, tasks.size()/taskLevelSet.get(Integer.valueOf(depth-1)).size());
...
//				System.out.println(bLevel + lLevel + task.getTaskSize() / speed);
	...
	//23的puRank
	public void calcPURank24(double theta, Map<Integer, List<Task>> taskLevelSet){
		double speed = VM.SPEEDS[VM.FASTEST];
...
//			System.out.println(t.getName() +"\t"+t.getpURank());
	...
	//23的puRank
	public void calcPURank241(Map<Integer, List<Task>> taskLevelSet, Map<Integer, Double> levelWidthTrend){
		double speed = VM.SPEEDS[VM.FASTEST];
	    ...
	    while(tls.hasPrevious()) {  //逆序遍历	//从出口任务开始计算
	        ...
	        for(Task task : tasks) {
				...
				//计算任务的level比例
				double lLevel = levelWidthTrend.get(Integer.valueOf(depth));
	...
	public void calcTaskBLLevels25(double theta, double MPTrate) { 
		double speed = VM.SPEEDS[VM.FASTEST];
		
		for(int j = this.size()-1; j >= 0; j--){ //从出口任务开始计算rank
			double bLevel = 0;	
			Task task = this.get(j);
			for(Edge outEdge : task.getOutEdges()){
				Task child = outEdge.getDestination();
			...
			//计算任务的level比例
			double lLevel = 0;
			if(j == this.size()-1) //出口任务
				lLevel = 0;
			else {
				int currentLeveSize = 0;
				for(Edge outEdge : task.getOutEdges()){
					Task child = outEdge.getDestination();
					currentLeveSize += child.getInEdges().size(); 
				}
				lLevel =  Math.pow(theta, currentLeveSize/task.getOutEdges().size());
			}
...
//					currentLeveSize += child.getInEdges().size(); 
//				}
//				lLevel =  currentLeveSize/task.getOutEdges().size();
//			}
//			if(lLevel >= (this.maxParallel*MPTrate+1))
//				lLevel = 0;
//			else
//				lLevel = 1;
//			if(j == this.size()-1) //出口任务
//				lLevel = 0;
			...
			task.setbLevel(bLevel + task.getTaskSize() / speed + lLevel);
		}	
	}
	
	public void calcTaskPURank25(double theta, double MPTrate) { 
		double speed = VM.SPEEDS[VM.FASTEST];
		...
		for(int j = this.size()-1; j >= 0; j--){ //从出口任务开始计算rank
			...
			for(Edge outEdge : task.getOutEdges()){
				...
				if(theta != Double.MAX_VALUE){		// if theta = Double.MAX_VALUE, flag = 1
					double et = child.getTaskSize() / speed;
					double tt = outEdge.getDataSize() / VM.NETWORK_SPEED;
					double d = 1-Math.pow(theta, -et / tt);	//网络传输时间越大，d取值越接近于0
					if(d<random())
						flag = 0;
				}
		...
		public void startElement(String uri, String localName, String qName, Attributes attrs) {
			if(qName.equals("job")){
				...
				if(runtime == 0) //ID00616的runtime=0
					runtime = 0.0000001;
				Task t = new Task(id, runtime); //L-ACO; 
//				Task t = new Task(id, runtime*1000); //上一代实例m1,m3; 
				...
				t.averageET = times/VM.TYPE_NO;
				t.setFastET(t.getTaskSize()/VM.SPEEDS[VM.FASTEST]);
