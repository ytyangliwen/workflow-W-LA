...
public class Solution extends LinkedHashMap<VM, LinkedList<Allocation>> implements Serializable{
	...
	public HashMap<VM, Task> vmMapTasks = new HashMap<VM, Task>();
...
//		HashMap<Task, Allocation> revMapping2 = newS.getRevMapping();
		for(VM vm : s1.keySet()) {
			...
			for(Allocation allo : s1.get(vm)) {
				...
				newS.vmMapTasks.put(newAllo.getVM(), newAllo.getTask());
			}
...
//		revMapping.put(alloc.getTask(), alloc);
	...
	public void addTaskToVM(VM vm, Task task, double startTime, boolean isEnd){
		...
		vmMapTasks.put(alloc.getVM(), alloc.getTask());
	...
	//for updateSubD
	public double calcSubD(Task task, VM vm){
		double EST = 0; 			
		for(Edge inEdge : task.getInEdges()){
			Task parent = inEdge.getSource();
			Allocation alloc = revMapping.get(parent);
	...
	public double calcSubD(Task task){
		double EST = 0; 			
		for(Edge inEdge : task.getInEdges()){
			Task parent = inEdge.getSource();
			Allocation alloc = revMapping.get(parent);
...
//		Collections.sort(list);			//把该solution当中的task以时间顺序排列，并检测是否是拓扑排序
	...
	public double getVMLeaseStartTime(VM vm){	
		if(this.get(vm).size() == 0)
			return 0; //return VM.LAUNCH_TIME;
		else{
			...
			return ftStartTime - maxTransferTime - VM.LAUNCH_TIME; //return ftStartTime - maxTransferTime;
		}
	}
